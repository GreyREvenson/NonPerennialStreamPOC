        ...
        if len(str(namelist.options.domain_hucid)) > 0:
            ...
            try:
                if isinstance(namelist.options.huc_break_lvl,int) and namelist.options.huc_break_lvl in (2,4,6,8,10,12): # override if break level specified
                    ...
                    colnam = f'huc{lvl}'
                    domain = pygeohydro.watershed.huc_wb_full(lvl)['huc'+str(lvl)]
                    domain = domain[domain['huc'+str(lvl)].str.startswith(namelist.options.domain_hucid)]
                else:
                    lvl    = int(len(namelist.options.domain_hucid))
                    colnam = f'huc{lvl}'
                    domain = pygeohydro.WBD(f'huc{lvl}').byids(field=f'huc{lvl}',

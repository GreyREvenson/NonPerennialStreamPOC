...
def _set_soil_transmissivity(namelist:twtnamelist.Namelist):
    ...
    if not os.path.isfile(namelist.fnames.soil_transmissivity) or namelist.options.overwrite_flag:
        if not os.path.isfile(namelist.fnames.soil_texture): sys.exit('ERROR could not find '+namelist.fnames.soil_texture)
        ...
        with rasterio.open(namelist._get_dummy_hrwtd_fname(namelist),'r') as wtd_highres:

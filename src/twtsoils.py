...
def _set_soil_texture_main(namelist:twtnamelist.Namelist):
    ...
    if not all([os.path.isfile(fname) 
                for fname in hucs['fname_soil_texture'].to_list()]):
        print('ERROR _set_soil_texture failed for hucs')
        for hucid in [hucid for hucid,row in hucs.iterrows()
                      if not os.path.isfile(row['fname_soil_texture'])]: 
            print(f'  {hucid}')
        sys.exit(0)
...
def _set_soil_transmissivity_main(namelist:twtnamelist.Namelist):
    ...
    if not all([os.path.isfile(fname) 
                for fname in hucs['fname_soil_transmissivity'].to_list()]):
        print('ERROR _set_soil_transmissivity failed for hucs')
        for hucid in [hucid for hucid,row in hucs.iterrows()
                      if not os.path.isfile(row['fname_soil_transmissivity'])]: 
            print(f'  {hucid}')
        sys.exit(0)

def _set_soil_texture(huc:geopandas.GeoDataFrame):
    import sdapoly,sdaprop
    """Get soil texture - uses pysda via https://github.com/ncss-tech/pysda.git"""
    soils_aoi = sdapoly.gdf(huc)
    ...
    soils_texture.to_crs(huc.crs).to_file(huc.iloc[0]['fname_soil_texture'], driver="GPKG") #'EPSG:4326'

def _set_soil_transmissivity(huc:geopandas.GeoDataFrame):
    dt_transmissivity = {'clay heavy'    :3.2,
                        ...
                        'sand'           :2.1,
                        'organic'        :2.5}
    soil_texture = geopandas.read_file(huc.iloc[0]['fname_soil_texture'])
    def calc_f(row):
    ...
    with rasterio.open(huc.iloc[0]['fname_dem'],'r') as dem:
        ...
        meta.update({"driver"    : "GTiff",
                     ...
                     "dtype"     : rasterio.float32,
                     "nodata"    : numpy.nan})
        with rasterio.open(huc.iloc[0]['fname_soil_transmissivity'], 'w', **meta) as dst:
